...
export default class FilesController {
  static async postUpload(req, res) {
    try {
      ...
      // construct key to get userId from redisClient
      ...
      // Update newFile document with localPath
      newFile.localPath = localPath;
      // logging to console for debugging
      console.log('newFile.localPath" ', newFile.localPath);

      // Add new file document in collection files
      const dbResult = await dbclient.filesCollection.insertOne(newFile);
      // logging to console for debugging
  ...
  static async getShow(req, res) {
    try {
      ...
      // Retrieve file document based on ID and user
  ...
  static async getIndex(req, res) {
    try {
      ...
      // set key for retrieving userId from redis
      const key = `auth_${token}`;
      ...
      // return files with status code 200
  ...
  static async putPublish(req, res) {
    try {
      ...
      // Define query to fetch fileDocument
      const toFetch = { userId: new ObjectId(user._id) };

      // Find fileDocument in filesCollection
  ...
  static async getFile(req, res) {
    ...
    try {
      ...
      // Check if  file is public or user is authenticated and owner
      ...
      // Check if file is locally present (replace with your own logic)
      if (!file.localPath) {
        return res.status(404).json({ error: 'File not found' });
      }
      ...
      // Read file content and send it as  response
